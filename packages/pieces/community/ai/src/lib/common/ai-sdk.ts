...
export function createAIModel(
  params: CreateAIModelParams<false>
): Promise<LanguageModelV2>;
export function createAIModel(
  params: CreateAIModelParams<true>
): Promise<ImageModel>;
  ...
  switch (provider) {
    ...
    case AIProviderName.CLOUDFLARE_GATEWAY: {
      const { accountId, gatewayId } =
        config as CloudflareGatewayProviderConfig;
    ...
    case AIProviderName.APIPIE: {
      const { apiKeyHeader, baseUrl } =
        config as OpenAICompatibleProviderConfig;

      const provider = createOpenAICompatible({
        name: 'openai-compatible',
        baseURL: baseUrl ?? 'https://apipie.ai/v1',
        headers: {
          'Content-Type': 'application/json',
          [apiKeyHeader ?? 'x-api-key']: auth.apiKey,
        },
      });
